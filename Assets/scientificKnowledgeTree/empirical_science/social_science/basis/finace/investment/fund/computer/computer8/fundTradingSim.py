...
#    print(list_b[0:3])
    year=int((parser.parse(list[5]).year))
    month=int((parser.parse(list[5]).month))
    day=int((parser.parse(list[5]).day))
    d1 = datetime.datetime(year, month, day)
    d2 = datetime.datetime.now()
    years = float((d2-d1).days)/365.0
    if (years < 3.0):
        rate_o = list_b[0][1];
    if (years > 3.0 and years < 5.0):
        rate_o = list_b[0][1]*(5/8.0)+list_b[1][1]*(3/8.0)
    if (years > 5.0 and years < 7.0):
        rate_o = list_b[0][1]*(7/15.0)+list_b[1][1]*(5/15)+list_b[2][1]*(5/15)
    if (years > 7.0 and years < 9.0):
